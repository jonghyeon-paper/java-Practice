...
public class ExcelHandler {
    ...
    public <T> XSSFWorkbook createXlsx(List<T> dataList, List<WritingProperty<T>> propertyList) {
        ...
        if (titleCreationFlag) {
            ...
            for (int cellIndex = 0; cellIndex < propertyList.size(); cellIndex++) {
                WritingProperty<T> property = propertyList.get(cellIndex);
        ...
        for (T item : dataList) {
            ...
            for (int cellIndex = 0; cellIndex < propertyList.size(); cellIndex++) {
                Object value = propertyList.get(cellIndex).getCombiner().apply(item);
                if (value == null) {
                    continue;
                }
    ...
    public <T> byte[] createXlsxBinary(List<T> dataList, List<WritingProperty<T>> propertyList) {
    ...
    private Method findSetterMethod(ReadingProperty property, Class<?> clazz) {
        try {
    ...
    public <E> List<E> createData(InputStream inputStream, List<ReadingProperty> propertyList, Class<E> clazz) {
        if (inputStream == null) {
        ...
        Map<Integer, ReadingProperty> propertyIndexMap = new HashMap<>();
                ...
                for (int rowIndex = 0; rowIndex < sheet.getPhysicalNumberOfRows(); rowIndex++) {
                    ...
                    if (rowIndex == 0) {
                        for (int cellIndex = 0; cellIndex < row.getPhysicalNumberOfCells(); cellIndex++) {
                            ...
                            for (ReadingProperty property : propertyList) {
                    ...
                    try {
                        ...
                        for (int cellIndex = 0; cellIndex < row.getPhysicalNumberOfCells(); cellIndex++) {
                            ...
                            ReadingProperty columnProperty = propertyIndexMap.get(cellIndex);
